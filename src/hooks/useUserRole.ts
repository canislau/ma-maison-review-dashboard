...
export function useUserRole(): { role: UserRole; name: string; email: string } {
  ...
  if (emailSet(import.meta.env.VITE_GOOGLE_ADMIN_EMAILS as string).has(email)) role = "Administrator";
  else if (emailSet(import.meta.env.VITE_GOOGLE_MANAGER_EMAILS as string).has(email)) role = "Manager";
