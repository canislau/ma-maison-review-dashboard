// ============================================================================
// Typed API client. Every call acquires a fresh access token via MSAL
// (silently refreshing when needed) and attaches it as a Bearer token.
// ============================================================================
...
async function getAccessToken(): Promise<string> {
  if (!msalInstance) throw new Error("API client not initialised — call initApiClient() first.");
  const account = msalInstance.getActiveAccount() || msalInstance.getAllAccounts()[0];
  if (!account) throw new ApiClientError(401, "NOT_SIGNED_IN", "Please sign in to continue.");
