...
function SignInScreen() {
  return <div className="min-h-screen flex items-center justify-center bg-section"><div className="card max-w-sm w-full text-center"><div className="w-12 h-12 rounded-full bg-accent-100 mx-auto mb-4 flex items-center justify-center text-accent-700 font-semibold text-xs">CWX</div><h1 className="text-lg font-semibold text-ink mb-1">CWX Review Dashboard</h1><p className="text-sm text-ink-muted mb-6">Sign in with your authorised work account.</p><div className="space-y-3"><GoogleSignInButton /><div className="flex items-center gap-3 text-xs text-ink-muted"><span className="h-px bg-border flex-1"/><span>or</span><span className="h-px bg-border flex-1"/></div><MicrosoftSignInButton /></div></div></div>;
}
...
function AppShell() {
  ...
  return <div className="min-h-screen"><header className="bg-surface border-b border-border sticky top-0 z-20"><div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4"><div className="flex items-center gap-3 min-w-0"><div className="w-9 h-9 rounded-lg bg-accent-600 text-white flex items-center justify-center font-semibold text-xs">CWX</div><div><h1 className="text-sm font-semibold text-ink">CWX Review Management</h1><p className="text-xs text-ink-muted">{name} · {role}</p></div></div><button className="btn-secondary" onClick={signOut}>Sign out</button></div><nav className="max-w-7xl mx-auto px-4 sm:px-6 flex gap-1 -mb-px overflow-x-auto">{TABS.map((tab) => <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`px-4 py-2.5 text-sm font-medium border-b-2 whitespace-nowrap ${activeTab === tab.key ? "border-accent-600 text-accent-700" : "border-transparent text-ink-muted"}`}>{tab.label}</button>)}</nav></header><main className="max-w-7xl mx-auto px-4 sm:px-6 py-6">{activeTab === "overview" && <OverviewTab />}{activeTab === "concern" && <ConcernReviewsTab />}{activeTab === "all" && <AllReviewsTab />}</main></div>;
}
