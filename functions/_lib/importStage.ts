...
export async function stageImport(env: Env, value: unknown, ttlSeconds: number): Promise<string> {
  ...
  const payload = encoder.encode(JSON.stringify({ expiresAt: Date.now() + ttlSeconds * 1000, value }));
  const encrypted = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, await encryptionKey(env.SESSION_SECRET), payload);
...
export async function readStagedImport<T>(env: Env, token: string): Promise<T | null> {
  ...
  try {
    ...
    const parsed = JSON.parse(decoder.decode(decrypted)) as { expiresAt: number; value: T };
