...
export const onRequest = withAuth(async ({ request, env, user }) => {
  if (request.method === "GET") {
    ...
    const items = await getAllListItems<SPActionTrackerFields>(env, env.ACTION_TRACKER_LIST_ID);
  ...
  if (request.method === "POST") {
    ...
    const created = await createListItem<SPActionTrackerFields>(env, env.ACTION_TRACKER_LIST_ID, fields);
