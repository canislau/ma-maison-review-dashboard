...
export const onRequest = withAuth<{ id: string }>(async ({ request, env, params, user }) => {
  ...
  await updateListItem<SPActionTrackerFields>(env, env.ACTION_TRACKER_LIST_ID, params.id, fields);

  const refreshed = await getListItemById<SPActionTrackerFields>(env, env.ACTION_TRACKER_LIST_ID, params.id);
