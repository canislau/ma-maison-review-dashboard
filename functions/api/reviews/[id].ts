...
export const onRequest = withAuth<{ id: string }>(async ({ request, env, params, user }) => {
  ...
  if (request.method === "GET") {
    const item = await getListItemById<SPReviewFields>(env, "Reviews", id);
  ...
  if (request.method === "PUT") {
    ...
    await updateListItem<SPReviewFields>(env, "Reviews", id, spFields);

    const refreshed = await getListItemById<SPReviewFields>(env, "Reviews", id);
  ...
  if (request.method === "DELETE") {
    ...
    await deleteListItem(env, "Reviews", id);
