...
export const onRequest = withAuth(async ({ request, env }) => {
  ...
  const outletFilteredOnly = filterReviews(allReviews, { outlet: filters.outlet }); // for outlet comparison table (ignores month so it's stable context)
  ...
  const data: DashboardData = {
    ...
    ...computeTrends(filtered.length > 0 ? allReviews : allReviews, 12),
    outlets,
  };
